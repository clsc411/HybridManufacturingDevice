...
class DispenseApp:
    def __init__(self, root):
        ...
        manual_entry = tk.Entry(root, textvariable=self.manual_var, width=12)
        manual_entry.grid(row=row, column=1, sticky="w", **pad)
        ...
        row += 1
        tk.Label(root, text="Dead volume (mL):").grid(row=row, column=0, sticky="w", **pad)
        self.dead_var = tk.StringVar(value=str(DEFAULT_DEAD_ML))
        tk.Entry(root, textvariable=self.dead_var, width=8).grid(
            row=row, column=1, sticky="w", **pad)

        # ── Send Button ────────────────────────────────────────────────
        row += 1
        self.send_btn = tk.Button(
            root, text="Send to Arduino", command=self._send_to_arduino,
            bg="#4CAF50", fg="white", height=2, width=20, state="disabled")
        self.send_btn.grid(row=row, column=0, columnspan=3, pady=12)

        # ── Status Bar ─────────────────────────────────────────────────
        row += 1
        self.status_var = tk.StringVar(value="Ready. Browse for an STL file or enter volume manually.")
    ...
    def _get_dead_ml(self):
        try:
            return float(self.dead_var.get())
        except ValueError:
            return DEFAULT_DEAD_ML

    def _update_results(self, cavity_ml, outer_ml, solid_ml):
        overfill = self._get_overfill()
        dead_ml = self._get_dead_ml()
        target = cavity_ml * (1 + overfill) + dead_ml
        ...
        self.result_labels["Dead volume"].set(f"{dead_ml:.2f} mL")
    ...
    def _browse_stl(self):
        ...
        # Run computation in a thread so the UI doesn't freeze
        def compute():
            try:
    ...
    def _use_manual(self):
        ...
        # Clear STL results and show manual value
